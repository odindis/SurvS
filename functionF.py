# Import Mdoules ______________________________________________________________
...
def add_inds_to_pop( population, num_max_inds, range_nz_wg, 
    ...
    #
    ## Weight genes part
    #  Randomly generate the number of non-zero values
    rnw = range_nz_wg
    num_nonzero = np.random.randint( rnw[0], rnw[1]+1, size = num_miss )
    # Randomly generate the positions of non-zero values for each row based 
    # on the number of non-zero values
...
def wrapper(args):
    idx, interval_group, score, is_train, size_min, is_warmup = args
    data_slice = GLOBAL_DATA[idx] if is_train else GLOBAL_DATA
    return calculate_difference(
        data=data_slice,
        interval_groups=interval_group,
        score=score,
        is_train=is_train,
        size_min=size_min,
        is_warmup=is_warmup,
...
def detector( data, population, list_values_decison, list_p, size_min, n_jobs, is_warmup=False ):     
    ...
    #
    return chromosomes, p_chromosomes, locs_target
#
#           
class loader:
    #
    ...
    #
    def save_chromosomes( self, path_, groups ):
        # np.save( path_, self.list_chromosomes )
        
        new_col = np.ones((self.list_chromosomes.shape[0], 1)) * groups
        chromosomes = np.hstack([self.list_chromosomes, new_col])
        
        np.savetxt( path_, chromosomes, delimiter=",", fmt="%g")
...
def get_indices_parents( population, prob_parent ):
    ...
    # individual probability become parents
...
def mutation( population, num_genes_weight, prob_mutation_weight_gene, 
    ...
    # Keep first column values less than second column values
