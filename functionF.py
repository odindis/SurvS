...
def calculate_individuals_fitness( population, data, num_genes_decision,
    ...
    if n_jobs == None or n_jobs == 1:
        ...
        if is_train:
            # list_p = Parallel( n_jobs = n_jobs, prefer='threads' )(
            ...
            with Pool(processes=n_jobs, initializer=init_worker, initargs=(data,)) as pool:
                list_p = pool.map(wrapper, [
                    (list_indices_sorted[i], list_interval_groups[i], list_score[i],
                    is_train, size_min, is_warmup)
                    for i in range(len(list_score))
                ])
